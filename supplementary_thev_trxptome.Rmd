...
flextable(supp_pcr_meth_tab) %>%
  ...
  colformat_image(j = "gel_image", i = c(1, 3:7, 9, 10), na_str = "-",
                  width = 1, height = 1) %>%
  flextable::width(j = ~forwardP, width = 1) %>%
  flextable::width(j = ~reverseP, width = 1) %>%
  # flextable::width(j = ~valid_status, width = 0.35) %>%
  flextable::width(j = ~gel_image, width = 1) %>% 
  footnote(., i = c(1:6, 7, 9, 10), j = 12, value = as_paragraph(c("Primer binds exon 1")),
           ref_symbols = c("a"), part = "body") %>%
  footnote(., i = c(1:6, 7, 9, 10), j = 13, value = as_paragraph(c("Primer binds terminal exon")),
